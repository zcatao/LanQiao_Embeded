...
{ 	 
	...
	delay_us(6);     //XPT2046 的转换时间最长为6us
	T_CLK_SET;//给1个时钟，清除BUSY   	    
	T_CLK_CLR; 
...
//读取一个坐标值(x或者y)
//连续读取READ_TIMES次数据,对这些数据生序排列
//然后去掉最低和最高位LOST_VAL个数,取平均值
//xy:指令(CMD_RDX/CMD_RDY)
//返回值:读到的数据
#define READ_TIMES  10  //读取次数
#define LOST_VAL  2	    //丢弃值最低最高几个
u16 TP_Read_XOY(u8 XY)
{
	u16 i, j;
	u16 buf[READ_TIMES];
	u16 sum=0,temp;	
	for(i=0;i<READ_TIMES;i++)//读 READ_TIMES 次
	{				 
		buf[i]=TP_Read_AD(XY);	    
	}		
...
u8 TP_Read_XY2(u16 *x,u16 *y) 
{
    ...
    if(flag==0)return(0);
    flag=TP_Read_XY(&x2,&y2);//读取两次XY坐标	   
    if(flag==0)return(0); //两次坐标在误差之内 可以返回1 并求取平均值 
...
void TP_Drow_Touch_Point(u16 x,u16 y,u16 color)
{
	POINT_COLOR=color;
	//起点xy  长度 方向  
	LCD_DrawLine1(x-12,y,x+13,y);//横线
	LCD_DrawLine1(x,y-12,x,y+13);//竖线
...
{		  
	s32 tempfac;			  
	u8 temp;
	temp=AT24C02_ReadOneByte(SAVE_ADDR_BASE+13);//读取标记字,看是否校准过！ 		 
	...
	{    												 
		...
		tp_dev.yoff=AT24C02_ReadLenByte(SAVE_ADDR_BASE+10,2);				 	  
 		tp_dev.touchtype=AT24C02_ReadOneByte(SAVE_ADDR_BASE+12);//读取触屏类型标记
...
{	  
	POINT_COLOR=RED;
	LCD_ShowString(40,160,TFT_Width,TFT_Height,16,"x1:");
 	LCD_ShowString(40+80,160,TFT_Width,TFT_Height,16,"y1:");
 	LCD_ShowString(40,180,TFT_Width,TFT_Height,16,"x2:");
 	LCD_ShowString(40+80,180,TFT_Width,TFT_Height,16,"y2:");
	LCD_ShowString(40,200,TFT_Width,TFT_Height,16,"x3:");
 	LCD_ShowString(40+80,200,TFT_Width,TFT_Height,16,"y3:");
	LCD_ShowString(40,220,TFT_Width,TFT_Height,16,"x4:");
 	LCD_ShowString(40+80,220,TFT_Width,TFT_Height,16,"y4:");  
 	LCD_ShowString(40,240,TFT_Width,TFT_Height,16,"fac is:");     
	...
	LCD_ShowNum(40+24+80,220,y3,4,16);	//显示数值
 	LCD_ShowNum(40+56,TFT_Width,fac,3,16); 	//显示数值,该数值必须在95~105范围之内.
		...
		{	
			...
			{			   
				case 1:						 
					...
					TP_Drow_Touch_Point(TFT_Width-20,20,RED);	//画点2
					break;
				case 2:
 					TP_Drow_Touch_Point(TFT_Width-20,20,WHITE);	//清除点2
					TP_Drow_Touch_Point(20,TFT_Height-20,RED);	//画点3
					break;
				case 3:
 					TP_Drow_Touch_Point(20,TFT_Height-20,WHITE);			//清除点3
 					TP_Drow_Touch_Point(TFT_Width-20,TFT_Height-20,RED);	//画点4
					break;                                                                                                                                                                                                                                                          
				case 4:	 //全部四个点已经得到
					...
					{ 
						...
						{
							cnt=0;
							TP_Drow_Touch_Point(TFT_Width-20,TFT_Height-20,WHITE);	//清除点4
							TP_Drow_Touch_Point(20,20,RED);									//画点1
					...
					//正确了
					//计算结果
					tp_dev.xfac=(float)(TFT_Width-40)/(pos_temp[1][0]-pos_temp[0][0]);//得到xfac		 
					tp_dev.xoff=(TFT_Width-tp_dev.xfac*(pos_temp[1][0]+pos_temp[0][0]))/2;//得到xoff
 					tp_dev.yfac=(float)(TFT_Height-40)/(pos_temp[2][1]-pos_temp[0][1]);//得到yfac
					tp_dev.yoff=(TFT_Height-tp_dev.yfac*(pos_temp[2][1]+pos_temp[0][1]))/2;//得到yoff  
					if(abs(tp_dev.xfac)>2||abs(tp_dev.yfac)>2)//触屏和预设的相反了.
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(TFT_Width-20,TFT_Height-20,WHITE);	//清除点4
   	 					TP_Drow_Touch_Point(20,20,RED);								//画点1
						LCD_ShowString(40,26,TFT_Width,TFT_Height,16,"TP Need readjust!");
					...
					POINT_COLOR=BLUE;
					LCD_Clear(WHITE);//清屏
					LCD_ShowString(35,110,TFT_Width,TFT_Height,16,"Touch Screen Adjust OK!");//校正完成
					delay_ms(1000);
					TP_Save_Adjdata();  
 					LCD_Clear(WHITE);//清屏   
					return;//校正完成				 
...
{		
	...
	AT24C02_Init();
	if(TP_Get_Adjdata())return 0; //已经校准 并且已经取出了参数值
	else//未校准的 是第一次校准的
	{ 										    
		LCD_Clear(WHITE);
		TP_Adjust(); //屏幕校准  //可以加入按键使能校准！！
		TP_Save_Adjdata();
	}	
	return 1; 									 
}
